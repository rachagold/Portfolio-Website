    ...
    <header className={cn(
      ...
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => {
            const isPortfolio = link.name === 'Portfolio';
            return link.dropdown ? (
              <div key={link.name} className="relative group" onMouseEnter={() => setIsPortfolioOpen(true)}
                onMouseLeave={() => setIsPortfolioOpen(false)}
              >
                <button className={cn(
                  "flex items-center gap-1 text-sm uppercase tracking-widest transition-all cursor-pointer",
                  isPortfolio 
                    ? "bg-[#93312A] text-white px-5 py-2.5 rounded-full hover:bg-[#7D2924] shadow-sm transform hover:scale-105 active:scale-95" 
                    : "text-[#2D1F1C] hover:text-[#93312A] py-2"
                )}>
                  {link.name} <ChevronDown className={cn("w-4 h-4 transition-transform", isPortfolioOpen && "rotate-180")} />
                </button>
                <AnimatePresence>
                  {isPortfolioOpen && (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute top-full left-0 w-48 bg-[#F5F0E8] shadow-lg rounded-xl overflow-hidden border border-[#93312A]/10 py-2 mt-1">
              ...
              <Link key={link.name} to={link.path} className={cn(
                ...
                isPortfolio && "bg-[#93312A] text-white px-5 py-2.5 rounded-full hover:bg-[#7D2924] shadow-sm transform hover:scale-105 active:scale-95"
              )}>
                {link.name}
              </Link>
            );
                ...
                <div key={link.name}>
                  {link.dropdown ? (
                    <div className="space-y-4">
                      <div className={cn(
                        "text-xl font-serif uppercase tracking-widest px-4 py-2 rounded-xl",
                        isPortfolio ? "bg-[#93312A] text-white inline-block" : "text-[#2D1F1C]/50"
                      )}>{link.name}</div>
                      <div className="pl-4 space-y-4 border-l border-[#93312A]/20">
                        {link.dropdown.map(dropLink => (
                          <Link key={dropLink.name} to={dropLink.path} className="block text-2xl font-serif text-[#2D1F1C] hover:text-[#93312A] transition-colors">
                            {dropLink.name}
                          </Link>
                        ))}
                      </div>
                    ...
                    <Link to={link.path} className={cn(
                      ...
                      isPortfolio ? "text-[#93312A] font-bold" : "text-[#2D1F1C]"
                    )}>
                      {link.name}
                    </Link>
                  )}
