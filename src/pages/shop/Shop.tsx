...
export function Shop() {
  ...
  const categories = ['Prints', 'T-shirts', 'Totes'];
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-12">
      <AnimatedSection className="text-center mb-16">
        ...
        <p className="text-[#2D1F1C]/80 text-lg mb-8">Original art prints, t-shirts, and tote bags. Each product features artwork from the Geo Graphic series.</p>
        ...
        <div className="w-full md:w-64 flex-shrink-0 space-y-8">
                ...
                {categories.map(cat => (
                  <label key={cat} className="flex items-center gap-3 cursor-pointer group">
                    ...
                    <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${
                      filters.category.includes(cat) ? 'bg-[#779C91] border-[#779C91]' : 'border-[#93312A]/30 group-hover:border-[#93312A]'
                    }`}>
                      {filters.category.includes(cat) && <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7"/></svg>}
                    </div>
                    <span className="text-[#2D1F1C]">{cat}</span>
                  </label>
                ))}
              </div>
            )}
          ...
          <div>
            ...
            {expandedSections.price && (
              <div>
                <input type="range" min="0" max="50" value={filters.priceRange[1]} onChange={(e) => setFilters(prev => ({ ...prev, priceRange: [0, parseInt(e.target.value)] }))}
