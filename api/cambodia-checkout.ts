...
export default async function handler(req: any, res: any) {
  ...
  const itemRows = items && items.length > 0
    ? (items as CheckoutItem[]).map((item: CheckoutItem) => {
        const details = [
  ...
  const emailPayload = {
    ...
    subject: `[Cambodia Order] ${name} — Pending Payment`,
    html: `
      ...
      <hr>
      <p><em>The customer is currently progressing to the payment selection phase (Stripe or ABA Pay).</em></p>
    `,
  };
